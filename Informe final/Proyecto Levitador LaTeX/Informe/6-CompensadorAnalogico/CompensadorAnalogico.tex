...
\begin{equation*}
	G_{IL}(s) =\frac{I_L}{V_{IL_{ref}}}=\frac{6}{1+\frac{s}{12.17}}\:[\frac{A}{V}]
\end{equation*}

\begin{equation*} 
	G_{p}(M,s)=\frac{Y_g}{I_L}=-\sqrt{\frac{30}{M}}*\frac{1.201}{s^{2}-4900}\:[\frac{m}{A}]
\end{equation*}

\begin{equation*} 
	H_{estim}(s)=\frac{V_{estim}}{Y_{g}}=\frac{259.6}{(1+\frac{s}{1\:kr/s})}\:[\frac{V}{m}]
\end{equation*}

La transferencia de lazo cerrado ($TLC_{interna}$) del diagrama \ref{fig:diag-interno} queda definida como:

\begin{equation}
	TLC_{interna}=\frac{Y_g}{V_{ref_c}}=\frac{G_c*G_{IL}*G_p}{1+G_c*G_{IL}*G_p*H_{estim}}\:[\frac{m}{V}]
\end{equation}
...
\subsection{Análisis de estabilidad}\label{sec_analisis_estab_analog}
...
%Se elige este valor para priorizar la estabilidad para el caso de masa máxima, con la desventaja de que se obtiene un menor margen de fase para el caso de masa mínima. 
...
\subsection{Verificación de estabilidad con masa mínima}
...
\begin{equation}
	TLC_{interna}(s)=\frac{Y_g}{V_{ref_c}}=\frac{G_c*G_{p}*G_{iL}}{1-G_c*G_{p}*G_{iL}*H_{estim}}\:[\frac{m}{V}]
	%	\frac{-3.6301*}{den}
\end{equation}
...
\noindent Para el an\'{a}lisis se considera como realimentación: 

\begin{equation*}
	H_{estim}=\frac{V_{estim}}{Y_g} = \frac{259.6}{(1 + \frac{s}{1\:k})
}\:[\frac{V}{m}]
\end{equation*}
...
\begin{equation}
	TLC_{externa}(s)=\frac{Y_g}{V_{y_{ref}}}=\frac{F*G_{ext}*TLC_{interna}}{1-G_{ext}*TLC_{interna}*H_{estim}}\:[\frac{m}{V}]
	%	\frac{-3.6301*}{den}
\end{equation}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{Etapa-de-entrada.png}
...
\begin{equation*} 
	R_5=\frac{R_1*R_3}{R_1*1.015-R_3}
\end{equation*}

Se elije $R_1=R_3=1\:k\Omega$, resultando en $R_5=66.7\:k\Omega$.
...
A continuación se plantea cómo está formada la salida del circuito con respecto a cada una de sus entradas. Para simplificar la escritura de las ecuaciones se define $X_{300}$ como el paralelo entre $R_{300}$ y la reactancia de $C_{300}$ y $\tau 0$ como el producto entre $R_{300}$ y $C_{300}$:
...
Si se considera que la tensión de $2.5\:V$ en la entrada no inversora es continua, se puede simplificar el término $X_{300}$ como una constante de valor $R_{300}$. Por lo tanto, se reescribe la ecuación \ref{eq_salida_integrador} como:
...
Es importante notar que la transferencia del circuito agrega una inversión de signo que no se encontraba presente en la transferencia del controlador diseñado. Por lo tanto, debe cancelarse en la siguiente etapa, que corresponde al sumador del lazo de realimentación interno.
...
En esta sección se realizará el diseño del compensador del lazo de control interno. Este puede ser dividido en tres etapas: dos redes de adelanto de fase conectadas en cascada seguidas de una ganancia.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{Red-adelanto-fase.png}
...
\begin{equation} 
	\frac{1}{C_x R_x} =  902.1\:[rad/s]
\end{equation} 

\begin{equation} 
	\frac{1}{C_x [R_x + R_1]} = 44.3\:[rad/s]
\end{equation} 
...
Finalmente aproximando la transferencia de lazo cerrado interna $TLC_{af}$ a $1/H$ y multiplicando la entrada por el bloque $G^-$ se obtiene una transferencia:
...
Por lo tanto, para tener un polo en $902.1\:[rad/s]$ y un cero en $44.3\:[rad/s]$, al elegir un capacitor $C = 1\:uF$, resulta en $R_x = 1100\:\Omega$ y $R_1 = 21.5\:k\Omega$. Además, se elige $R_a = R_b = 200\:k\Omega$ para obtener una ganancia unitaria. 
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.45]{circuito_completo_comp-an.png}
