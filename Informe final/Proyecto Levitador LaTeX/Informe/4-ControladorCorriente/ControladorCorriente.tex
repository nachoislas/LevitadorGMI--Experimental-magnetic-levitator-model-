...
Se debe diseñar un circuito restador con entradas variables entre $0\:V$ y $5\:V$ cuya salida tenga un punto de operación $V_{ope}$ y que, además, multiplique la tensión de referencia por la ganancia $K_{in}$. Para ello se propone una topología con un amplificador operacional y resistencias como la que se muestra en la figura \ref{fig:img_etapa-de-entrada}. 
...
Estas ecuaciones se cumplen para valores de resistencias que sean todas iguales, es decir $R_{14}=R_{15}=R_{16}=R_{17}$. Por lo tanto se elige que estas resistencias tengan el valor $10\:kOhm$.
...
Para saber qué valor utilizar para el \textsl{offset} se debe calcular el mínimo de tensión que podría entregar el sensor de efecto Hall. Esto se da cuando la referencia del controlador de corriente es de $0\:V$, con lo cual la corriente media del electroimán es $0 \:A$ con una excursión de $±250\:mA$. Lo que significa una tensión de salida del sensor de efecto Hall de $13.3\:mV$ por encima y debajo del \textsl{offset}. Para tomar un margen se incrementa a $V_{op}=2.6\:V$. \colorbox{red}{habría que aclarar mejor cuales quedan en 2.5 y uales en 2.6}
