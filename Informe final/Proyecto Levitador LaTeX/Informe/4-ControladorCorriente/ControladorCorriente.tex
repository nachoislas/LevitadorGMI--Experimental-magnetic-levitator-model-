...
Como se ve en la tabla en los $BJT$ al circular altas corrientes las pérdidas en $V_{ce}$ son altas (en torno a $1 V$) en cambio los $MOSFET$ presentan una $R_{ds}$ parásita que puede ser del orden de los miliOhms. Esto hace que para una corriente de $30 A$ y una $V_{CC}$ de $24 V$ la caída $V_{ds}$ a comparación de la $V_{ce}$ de los $BJT$ sea mucho menor. Además los $MOSFET$ poseen un diodo anti paralelo que permite el flujo de corrientes parásitas en sentido opuesto. Esto es útil para corrientes espurias que se generan por la conmutación en el puente H.
