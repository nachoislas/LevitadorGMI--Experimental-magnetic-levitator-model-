...
Si bien la pendiente cambia según las condiciones iniciales, para este análisis se considera $I_o=0\:A$ y luego se analizará cómo afecta esta suposición en el sistema real. De esta forma, la pendiente resulta:
...
La señal $V_{sns}$ está dada por la siguiente ecuación:
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=1]{bjt_vs_mosfet.png}
...
\colorbox{red}{Cambiamos offset y setpoint por punto de operacion?}
...
Para su implementación se propone la topología circuital mostrada en la figura \ref{fig:img_frecuencia-auxiliar}. Este oscilador fue utilizado en la versión anterior del prototipo para generar a su salida una onda pulsada con un ciclo de trabajo variable entre dos posibles valores: $d=90\%$ y $d=10\%$, dependiendo del valor de su entrada: $0\:V$ o $5\:V$, respectivamente. Por lo tanto, se decide seguir con esta topología ajustando el valor de $C_1$ para obtener una frecuencia de conmutación de $F_{aux}=50\:kHz$..
...
Este oscilador funciona de la siguiente manera. Cuando la entrada es $V_h=0\:V$, resulta en un  ciclo de trabajo $d=90\%$. Es decir, del periodo total de la señal ($T_{total}=\frac{1}{F_{aux}}=20\:uS$), el 90\% del tiempo la salida ($V_{sw}$) se encuentre en $5\:V$, y el 10\% restante se encuentra en $0\:V$. Por lo tanto $T_{on}=0.9*T_{total}=18\:uS$ y $T_{off}=0.1*T_{total}=2\:uS$.

De igual forma, cuando la entrada es $V_h=5\:V$, resulta en un ciclo de trabajo $d=10\%$. Es decir, $T_{on}=0.1*T_{total}=2\:uS$ y $T_{off}=0.9*T_{total}=18\:uS$.

Finalmente, el capacitor $C_1$ se calcula como:

\begin{equation} 
	C_1= \frac{4.5*10^{-5}}{F_{aux}} = 900\:pF
\end{equation}

\colorbox{red}{lo volví a calcular mejor y me quedó de 1.22nF, ese valor anda mejor en la simulación. mñaana escribo la ecuación}

Por otro lado, es importante notar que este circuito presenta una inversión de fase a su salida. Por lo tanto, debe ser tenida en cuenta en el diseño final del lazo de control de corriente. De lo contrario, el sistema resultaría inestable y no se podría controlar el valor de corriente.
...
1- Cuando $V_e>V^+$ entonces $V_h=0\:V$. Por lo tanto:
...
Para poder obtener la señal $\overline{V_{SW}}$ a partir de $V_{SW}$, se utiliza el inversor SN74LVC3G06DCUT \colorbox{red}{Agregar hoja de datos}. Por lo tanto, la salida de este se conecta con las señales de entrada BHI y ALI del HIP4081A. Por otro lado, para las señales de entrada restantes (BLI y AHI), que no requieren inversión de signo, se podría ingresar directamente con la señal $V_{SW}$. Sin embargo, debido a que el inversor produce flancos de subida y de bajada abruptos, se decidió volver a invertir la señal $\overline{V_{SW}}$ y obtener una señal mas $V_{SW}$ con flancos de subida mas abruptos que la original. 
...
\colorbox{red}{ver si modificamos los nombres mas arriba como Fplanta, Faux, etc... y si ponemos una referencia de a donde los calculamos}
