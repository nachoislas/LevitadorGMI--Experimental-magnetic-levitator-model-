...
En la figura \ref{fig:img_MCP_sin_compensar} se muestra el diagrama de Bode de A(s) y la red de realimentación $\frac{1}{H(s)}$. En ella se puede observar que cuando ambas curvas se cruzan (G*H=1), la contribución en fase de ambas suman 180º, por lo tanto el circuito resulta inestable. 

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{MCP_sin_compensar.png}
	\caption{Respuesta en frecuencia del amplificador operacional MCP664 y de la red de realimentación sin compensar.}
	\label{fig:img_MCP_sin_compensar}
\end{figure}

Debido a la inestabilidad del circuito derivador es necesario compensarlos mediante el agregado de un polo en $\frac{1}{H(s)}$ de forma tal que, cuando ambas curvas se crucen, la suma de fase sea menor a 180º. Esto se logra agregando una resistencia ($R_2$) en serie al capacitor, como se observa en la figura \ref{fig:img_Circuito_derivador_compensado}. De esta forma, la red de realimentación resulta:

\begin{equation} \label{eq_MCP_compensado}
	\frac{1}{H(s)}=\frac{1+s*C_1*(R_1+R_2)}{1+s*C_1*R_2}\simeq \frac{1+s*C_1*R_1}{1+s*C_1*R_2}
\end{equation}
...
Al agregar esta compensación el circuito deja de comportarse como derivador a partir de la frecuencia del polo agregado en $\frac{1}{H(s)}$. Por lo tanto, para determinar en qué frecuencia ubicarlo, se debe tener en cuenta hasta qué frecuencia se desea que el circuito presente un comportamiento derivativo. De la tabla \ref{tab_mediciones} se obtiene que la frecuencia máxima a la que trabaja el sistema (para $Y_g=5\:mm$) es de $1632.35\:Hz$. Sin embargo, debido a que la forma de onda es triangular, se considera que se debe




 se debe diseñar la compensación teniendo en cuenta esto. Se sabe que la onda triangular de la corriente, aunque tiene frecuencia variable, su frecuencia máxima en el rango de trabajo, obtenida de la tabla \ref{tab_mediciones} para $Y_g=5\:mm$, es $1632.35\:Hz$. Se desea que el circuito presente comportamiento derivativo hasta por lo menos su quinta armónica, es decir hasta $8161\:Hz$.

Por lo tanto se ubica el polo en $16 \:kHz$, que da como resultado $R_2=10\:\Omega$, $C_1=1\:uF$ y $R_1=25\: k\Omega$. El bode de la ganancia a lazo abierto del amplificador operacional junto con su red de realimentación $\frac{1}{H(s)}$ con el cero agregado se observa en la figura \ref{fig:img_MCP_compensado}.
...
En la figura \ref{fig:img_GH del derivador compensado} se muestra la magnitud y la fase transferencia a lazo abierto (G(s)*H(s)). En ella se puede observar que el margen de fase resultante es de $\phi =49.6{}^\circ $.
...
Finalmente, en la figura \ref{fig:img_Transferencia-de-lazo-cerrado} se muestra la transferencia de lazo cerrado (TLC), que tiene un comportamiento derivativo hasta aproximadamente $18 \:kHz$. 
 ...
 quinta armónica de la señal ($10 \:kHz$), como es deseado.
