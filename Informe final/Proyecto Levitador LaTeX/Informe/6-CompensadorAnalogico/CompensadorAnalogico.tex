...
Luego de la etapa de generación de $V_{y_{ref}}$ se encuentra una etapa de ganancia representada por el bloque F que se muestra en la figura \ref{fig:diag-en-bloques-comp_final} y luego se realiza la suma con la señal realimentada $V_{estim}$. Este circuito estará implementado a partir de la utilización de amplificadores operacionales alimentados con fuente simple de $5\:V$, al igual que en el resto de las etapas. Es por ello que se debe tener la consideración de que la salida de esta etapa tenga un punto de operación de $2.5\:V$. Por lo tanto el circuito debe implementar la función:

\begin{equation*} 
	Ve_{ext}= 2.5\:V + V_{estim} + F*V_{y_{ref}}
\end{equation*}

Como se diseñó en la sección \ref{sec_calculo_F}, el valor de F es negativo. Por lo tanto se puede reescribir la función como:
...
\end{figure}
\colorbox{red}{convendrá poner el seguidor en la etapa de generación de Vyref?? sino ya fue } Ya fue jeje
