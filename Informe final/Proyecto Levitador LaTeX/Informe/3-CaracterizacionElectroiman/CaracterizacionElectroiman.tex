...
\noindent  En este capítulo se detallan las características constructivas que posee el electroimán. Además, se realiza un modelado físico del sistema para encontrar una expresión que relaciona la fuerza magnética ejercida en función de la variable de control y de la variable que se desea controlar. Luego, a partir de este modelo, se obtienen otros parámetros relevantes para el diseño del sistema de control.

\section{Modelado matemático del electroimán}

En esta sección se van a desarrollar las ecuaciones matemáticas que describen el funcionamiento del electroimán. Para ello se van a analizar las fuerzas que se aplican a la pieza móvil y la naturaleza de las mismas.

Como se mencionó previamente, para el desarrollo del proyecto se utiliza un electroimán compuesto por dos piezas: una con forma de “E” y otra con forma de “I”.
...
\noindent A partir de la ecuación \ref{eq_inductancia_vs_y}, se realiza una expansión por serie de Taylor linealizada en torno al punto $Y_{g}=Y_{0}=4\:mm$. Se desprecian los términos de orden mayor o igual a 2, con el objetivo de llegar a una expresión lineal para la inductancia. Se obtiene:
...
Se puede ver que la ecuación obtenida no difiere demasiado con la calculada en la sección \ref{secc_exp_ind_linealizada}. Las variaciones en los valores obtenidos con respecto a la calculada de forma teórica están dados por las desviaciones en los materiales utilizados para construir el electroimán. Se utilizará como aproximación de la inductancia a la calculada de forma practica (ecuación \ref{eq_induct_practica}).
