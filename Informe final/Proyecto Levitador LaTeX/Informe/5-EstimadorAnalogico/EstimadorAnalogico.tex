...
En la expresión \ref{eq_Yg_despejada} se puede observar que para obtener una estimación de la distancia de separación es necesario medir el módulo de la pendiente de la onda triangular de la corriente. Para ello, se implementa un circuito derivador seguido de uno que permita obtener el módulo.

Debido a que la forma de medir la corriente del electroimán es a través del sensor de efecto Hall es necesario tener en cuenta su ganancia $H_0$. Por lo tanto, la expresión a implementar circuitalmente resulta:

\begin{equation} \label{eq_Yg_salida_sensor}
	Y_g=\frac{5.136*10^{-6}}{H_0}*{\left|\frac{dV_{IL}}{dt}\right|}_{Lineal} - 3.472*10^{-3}
\end{equation}
...
Por lo tanto, se plantea el diagrama en bloques de la figura 111.
...
\end{figure}
\colorbox{red}{no se si los nombres estarán correctos, después los cambiamos}
