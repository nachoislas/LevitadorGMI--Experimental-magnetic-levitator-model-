...
Para poder diseñar un controlador de corriente apropiado es necesario realizar una medición sobre la corriente que circula por el bobinado del electroimán y actuar en consecuencia. Es necesario conocer tanto el valor medio de la corriente, como su ripple. Es por ello que se debe idear una estrategia de medición que represente correctamente esta forma de onda cuyo valor medio puede alcanzar valores desde 0 hasta 30 A con un ripple de 500 mA.

En esta sección se analizan dos alternativas para lograr este objetivo. La primera mediante una resistencia y la segunda con un sensor de efecto Hall.

\subsubsection{Análisis de medición de corriente mediante resistencia shunt}
Una posible opción para efectuar la medición de corriente es colocar una resistencia shunt en serie con el electroimán y medir la diferencia de tensión en sus terminales. Para no generar pérdidas de potencia considerables, se elige una resistencia de 10mOhms. Debido a que los terminales de la resistencia se encuentran a una tensión flotante, es decir varía entre 0V y VCC se necesita implementar un amplificador que trabaje en modo diferencial, para obtener una salida de tensión referenciada a masa.

El principal problema se encuentra en que los puntos de medición del amplificador en modo diferencial cambian bruscamente en cada conmutación generando transitorios que introducen ruido de modo común a la misma frecuencia que la onda triangular que se quiere analizar. Por este motivo se propone utilizar otro tipo de sensor.

\subsubsection{Análisis de medición de corriente mediante Sensor de Efecto Hall}
Dado que la medición con una resistencia de shunt introduce ruido en modo común se plantea como opción la alternativa de utilizar dispositivos que miden variaciones de campo magnético producidas por la corriente, conocidos como sensores de efecto Hall. La ventaja de estos sensores es que al estar midiendo el campo magnético, generado por la circulación de corriente, las conmutaciones no afectan en la medición.

Existen una gran variedad de estos sensores en el mercado, cada uno con diferentes características. A continuación se mencionan los criterios que el sensor debe cumplir.
