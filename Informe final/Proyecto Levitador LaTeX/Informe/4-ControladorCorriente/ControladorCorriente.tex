...
Para la implementación de las llaves se analiza la utilización de dispositivos semiconductores pertenecientes a la clase transistores. En el mercado existe una amplia variedad de ellos, entre los que se encuentran los transistores bipolares de juntura (TBJ), los transistores de efecto de campo metal-oxido-semiconductor (MOSFET), entre otros. En la figura \ref{fig:img_bjt_vs_mosfet} se muestra el símbolo eléctrico de cada uno, con el nombre de cada uno de sus terminales. Cada uno de ellos posee características distintivas que lo hacen más o menos apropiado para cada aplicación en particular. 
...
Por lo tanto, se decide utilizar los MOSFET IPB160N04 ya que, ademas de cumplir con los requerimientos: soportan una $I_{D_{MAX}}=160\:A$, una tensión $V_{DS_{MAX}} = 40\:V$ y una $R_{ds} = 1.6 \:m \Omega$, fueron probados en el prototipo anterior. Estos transistores son de tipo N y, para que estén en estado de conducción con la $R_{ds}$ especificada anteriormente, debe aplicarse una tensión entre \textsl{gate} y \textsl{source} mínima de $V_{gs}= 7\:V$.
...
Para su implementación se propone la topología circuital mostrada en la figura \ref{fig:img_frecuencia-auxiliar}. Este oscilador fue utilizado en la versión anterior del prototipo para conmutar con un ciclo de trabajo del $d=90\%$. Por lo tanto, se decide seguir con esta topología ajustando el valor de $C_1$ para obtener una frecuencia de conmutación de $F_{aux}=50\:kHz$. Este circuito presenta, además, una inversión de fase a su salida.

Un ciclo de trabajo $d=90\%$ significa que del periodo total de la señal ($T_{total}=\frac{1}{F_{aux}}=20\:uS$), el 90\% del tiempo la señal de salida ($V_{sw}$) se encuentre en $5\:V$, y el 10\% restante se encuentre en $0\:V$. Por lo tanto $T_{on}=0.9*T_{total}=18\:uS$ y $T_{off}=0.1*T_{total}=2\:uS$.

\colorbox{red}{No habría que aclarar lo mismo para cuando se encuentra el 90\% en bajo?}
...
El capacitor $C_1$ se calcula como:

\begin{equation} 
C_1= \frac{4.5*10^{-5}}{F_{aux}} = 900\:pF
\end{equation}

Se menciona que este circuito invierte la fase ya que se pueden diferenciar dos estados según la tensión en su entrada:
...
Esta inversión de fase es importante ya que debe ser tenida en cuenta para terminar de diseñar el lazo de control de corriente. De lo contrario el sistema resultaría inestable y no se podría controlar el valor de corriente.
\colorbox{red}{ver como decir lo de la inversion de fase, lo repetimos mucho ahi arriba}
...
\colorbox{red}{Aclarar que componentes usamos para los inversores y agregar datasheet}

\colorbox{red}{para que se usan esos inversores}
