...
Al aplicar la transformada inversa de Laplace a la expresión  \ref{eq_corriente}, se puede obtener la respuesta temporal de la corriente ante un escalón de tensión con amplitud ``$v_L$'' en la entrada, considerando corriente inicial $I_o$ y constante de tiempo $\tau=\frac{L(Y_g)}{R_L}$.
...
De este diagrama surgen las siguientes necesidades:
\begin{itemize}
...
A continuación se analizan distintas alternativas para satisfacerlas.

\subsection{Diseño de la topología de la fuente de alimentación}
En esta sección se analizan las distintas topologías para la fuente de alimentación del electroimán. Inicialmente se analiza el funcionamiento de un controlador de corriente mediante el uso de un transistor trabajando en modo lineal y luego utilizando una fuente de alimentación trabajando en conmutación.
...
En este modo de funcionamiento la tensión colector-emisor ($V_{CE}$) se controla de manera que su diferencia con la tensión de alimentación, circulando por la resistencia interna del electroimán, genere la corriente deseada. Es decir, en régimen permanente se obtiene:
...
A diferencia del controlador mediante un transistor trabajando en modo lineal, en esta topología se logra una eficiencia extremadamente alta, ya que las llaves operan en corte y conducción, por lo que la disipación de potencia se produce solo en el electroimán y no en el circuito de control Debido a este motivo, se decide utilizar esta topología para la fuente de alimentación.
...
Para comenzar el diseño circuital es importante determinar cómo será la alimentación del controlador de corriente. Para definirla se tendrá en cuenta la velocidad de respuesta de la planta que está determinada por el polo dominante. Es decir el sistema debe ser lo suficientemente rapido de modificar el valor de la corriente a medida que el usuario se lo exige. Un caso a analizar es cuando en ragimen permanente se modifica bruscamente la carga que se esta levitando. En esta situacion el sistema debe aumentar o disminuir la corriente de forma abrupta para evitar que el objeto se caiga. El caso de mayor exigencia se da cuando a una distancia maxima de referencia Y=5mm se modifica de carga minima(1Kg) a maxima(30Kg). Utilizando la ecuacion XX se obtiene que la corriente para ambos casos es de:

\begin{equation}
	I_L(Y=4mm)[m=30Kg]=16.3 A
	I_L(Y=4mm)[m=1Kg]=2.9 A
\end{equation}
...
Finalmente reemplazando en \ref{eq_tiempo_de_subida} se obtiene: $Vcc>=13,72\:V$.
...
Considerando $\Delta Y=2\:mm$, se llega a que $T=14,27\:ms$. Por lo tanto se desea que $T1<T$.
