...
La señal que entra al ADC es la salida del sensor de efecto Hall $V_{IL_{feed}}$, que fue calculada en \ref{eq_salida_restador_hall_2} y se repite a continuación.
...
La conversión entre la señal $V_{IL_{feed}}$ y $V_h$ será realizada dentro del microcontrolador luego de muestrear la señal de entrada con el ADC.

La estimación de la distancia de entrehierro se obtendrá al medir la pendiente de la señal $V_h$. Dentro del microcontrolador se implementa un algoritmo para procesar las muestras y calcular la distancia estimada. 
...
\begin{table}[H]
	\begin{center}
		\begin{tabular}{| c | c | c |}
			\hline
			$Y_g\:[mm]$ & $L\:[mH]$ & $F_{planta}\:[Hz]$\\ \hline
			2 & 22.64 & 1060\\ \hline
			3 & 18.8 & 1276\\ \hline
			4 & 16.44 & 1459\\ \hline
			5 & 14.9 & 1632\\ \hline
...
\begin{equation} \label{eq_yg_vs_derivada}
	Y_g = 5.136*10^{-6}\:[\frac{m*s}{A}]*|\frac{di_L}{dt}|- 3.472*10^{-3} [m]
\end{equation}
...
Por otro lado, el valor de $L(Y_g)[n-1]$ se obtiene al aplicar el valor estimado de posición anterior en la ecuación \ref{eq_inductancia}. El cálculo de esta expresión se obtuvo a partir de la ecuación de la inductancia \ref{eq_induct_practica} en función de las mediciones realizadas sobre el electroimán.


\begin{equation} \label{eq_inductancia}
	L(Y_g)[n] = -2.56\:[\frac{H}{m}]*Y_g[n]+0.0271\:[H]
\end{equation}
...
\resizebox{.85\hsize}{!}
{
	$P(F_{planta}) = \frac{\Delta V}{T_{planta}/2} = 2*K_h*\Delta i_L*F_{planta} = 2 * 0.0533\:[\frac{V}{A}] * 0.5\:[A] * F_{planta}$
}
\end{equation}
...
\begin{equation} 
	P(F_{planta}) = 50 * [0.0533\:[\frac{V}{A}] * 0.5\:[A] * (F_{planta}*2)]\:[\frac{V}{s}]
\end{equation}
...
\subsubsection{Referencia de posición} \label{sec_referencia_pos}
 ...
 \begin{equation} \label{eq_vyref_vs_yg}
 	V_{y_{ref}}=259.6\:[\frac{V}{m}]*Y_g+3.4\:[V]
 \end{equation}
...
\begin{equation} \label{eq_y-ref-dig}
	Y_{ref}\ =\frac{Vy_{ref_{ADC}} +2.5\:[V] - 3.4\:[V]}{259.6\:[\frac{V}{m}]}\:[m]=\frac{Vy_{ref_{ADC}} - 0.9\:[V]}{259.6\:[\frac{V}{m}]}\:[m]
\end{equation}
...
Debido a la acción del cero en el origen, la ganancia del circuito irá en aumento hasta alcanzar la frecuencia del primer polo ubicado en $100\:Hz$. Una vez alcanzado, la ganancia permanecerá constante. Ese valor es la ganancia resultante en la banda de paso. Como se desea que sea de 50, se plantea la siguiente ecuación:
...
\begin{equation*} 
	\frac{R_{402}}{R_{400}} +1 = \frac{5\:[V]}{3.3\:[V]}
\end{equation*}
...
\begin{equation}
	TLC_{interna}=\frac{Y_g}{V_{ref_c}}=\frac{G_c*G_T}{1+G_c*G_T}\:[\frac{m}{V}]
\end{equation}
...
\begin{equation}
	TLC_{interna}(w)=\frac{Y_g}{V_{ref_c}}=\frac{G_c*G_T}{1-G_c*G_T}\:[\frac{m}{V}]
	%	\frac{-3.6301*}{den}
\end{equation}
...
\subsection{Diseño del lazo de realimentación externo}
...
\begin{figure}[H]
	\centering
	\scalebox{0.8}{\input{tikz/diagrama_general_digital_completo.tex}}
