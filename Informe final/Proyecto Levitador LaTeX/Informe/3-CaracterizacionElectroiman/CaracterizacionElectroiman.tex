...
Se desea mantener una levitación de manera estable. Por lo tanto, es necesario diseñar un sistema de control. Para ello, primero se debe caracterizar el comportamiento dinámico de la planta.

La ecuación \ref{eq_fuerza_magnetica} muestra que la fuerza magnética depende del cuadrado de la corriente y del cuadrado de la inversa de la distancia del entrehierro. Es decir, se trata de un fenómeno no lineal. La desventaja que esto conlleva es que resulta complejo el diseño de un compensador adecuado. Sin embargo, es posible aplicar técnicas de compensación lineales si se considera que el comportamiento de la planta es aproximadamente lineal en un cierto rango de operación.

Por lo tanto, para poder aplicar este tipo de técnicas de compensación, es necesario obtener un modelo lineal e invariante en el tiempo de la planta. Para ello se utiliza el método de linealización por representación en serie de Taylor y se encuentra el modelo de estados. Este proporciona una representación matemática de un sistema físico, en función de sus entradas, salidas y variables de estado, que se encuentran relacionadas por ecuaciones diferenciales de primer orden. Este modelo de estados lineal es representativo de la planta siempre que las variaciones de distancia del entrehierro estén limitadas a un rango de operación en el que se mantenga un comportamiento lineal.
