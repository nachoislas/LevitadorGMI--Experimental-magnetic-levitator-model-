...
Para ello, se plantea el caso en que el sistema se encuentre manteniendo en suspensión a un objeto con una masa de 1Kg a una distancia de 4mm y esta cambie abruptamente a 30Kg. 

Teniendo en cuenta que para mantener un objeto de 30Kg suspendido a una distancia de 4mm se requiere una coriente de 2233A y para 5mm 131A, el sistema debe poder aumentar su corriente 2332A antes de que el objeto supere 1mm

En este caso, el sistema debe ser capaz de aumentar la corriente que circula por el electroimán 



Para ello, se propone analizar el caso en que el levitador se encuentra funcionando con una distancia de entrehierro de 4mm y con el peso mínimo (1kg). El electroimán tiene una corriente media de:
\colorbox{red}{repetir cálculo con y=4}
\begin{equation}
	I_L(Y=4\:mm)[m=1\:kg]=3.72\:A  
\end{equation}

A continuación se aumenta abruptamente la carga, haciendo que m=30kg. Por lo tanto el compensador del lazo de control de distancia (supongamos que el controlador pone la referencia de corriente en la máxima). Aunque el controlador responda inmediatamente, la corriente en el electroimán tardará cierto tiempo en alcanzar el valor necesario, por lo tanto se espera que la distancia de separación aumente un cachito. Nos interesa que este aumento de distancia no sea mayor a 1mm, por lo tanto se necesita que la corriente sea:
