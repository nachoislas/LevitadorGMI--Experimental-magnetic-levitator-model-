...
\colorbox{red}{cambiar imagen por diagrama interno} 

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{Diagrama-en-bloques-comp.png}
	\caption{Diagrama simplificado del sistema completo.}
	\label{fig:diag-interno}
\end{figure}
...
Para realizar el análisis de estabilidad se parte de las transferencias de la planta $G_{p}(s)$ para una masa de $30\:kg$, la del controlador de corriente $G_{iL}(s)$ y la del lazo de realimentación $H_{estim}(s)$. A partir de ellas se obtiene  la transferencia a lazo abierto total $GH_T(s)$ mostrada en la expresión \ref{eq_GT2}.
...
\noindent Se plantea un lazo de realimentaci\'{o}n externo como se muestra en la  figura \colorbox{red}{poner referencia a imagen del diagrama simplificada}. En el lazo de realimentaci\'{o}n interno act\'{u}a el compensador por adelanto de fase dise\~{n}ado previamente y, en el externo, un controlador del tipo integral. De esta forma, se logra suavizar la respuesta al escal\'{o}n del sistema y eliminar el error en r\'{e}gimen permanente.
...
\noindent Se  pretende diseñar un compensador del tipo integrador cuya transferencia es de la forma:
\begin{equation}
	G_{integ}= \frac{k_{int}}{s}
\end{equation}

El problema de este integrador ideal es que presenta una ganancia infinita en contínua. Por lo tanto, se propone implementar un compensador con un polo $p_{int}$ en baja frecuencia, que actúe como integrador a las frecuencias de la planta, pero que tenga ganancia finita en contínua.  Sin embargo, esta modificación provoca que la cancelación del error en régimen permanente no sea completa. Por lo tanto, se decide ubicar el polo en $0.1\:rad/s$ y la transferencia a implementar resulta.
...
Para encontrar el valor adecuado de $k_{int}$ se grafica el lugar de raíces de la expresión \label{eq_cadena_avance_integrador} considerando $k_{int}=1$. En la figura \ref{fig:lugar-de-raices-con-integrador-analog} se muestra el lugar de raíces.

\colorbox{red}{hacer imagen de lugar de raices con realimentacin negativa}

Se puede observar que ningún valor de $k_{int}$ logra estabilizar el sistema. Por lo tanto para este lazo de realimentación externo también debe utilizarse realimentación positiva, puesto que la TLC interna del sistema presenta una ganancia negativa.
