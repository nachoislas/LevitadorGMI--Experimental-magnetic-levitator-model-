...
Entonces, con este ancho de histéresis elegido se obtiene una frecuencia de conmutación de la planta (calculada para el peor caso de corriente $I_{min}=30\:A$):
...
\begin{equation} \label{eq_kin}
	K_{in}=\frac{30\:[A]*H_0}{5\:[V]}=6\:[\frac{A}{V}]*H_0 
\end{equation}
...
A partir de estas características se decidió utilizar un sensor de efecto Hall detallado en \cite{HO15-NP}. Este permite medir una corriente de $\pm 37.5\:A$ con un ancho de banda de $250\:kHz$ y una transresistencia de $H(s)=H_0=53.33\:mV/A$ en todo  el rango de corriente. Además, presenta alta inmunidad a interferencias externas. Por otro lado, requiere una fuente de alimentación simple de $5\:V$.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{conexion_sensor_hall.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{bjt_vs_mosfet.png}
...
\begin{table}[H]
	\begin{center}
		\begin{tabular}{| c | c | c |}
			\hline
			\backslashbox{Característica}{Tipo} & $BJT$ & $MOSFET$\\ \hline
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{proteccion-mosfet.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{Bootstrap-con-cap.png}
	\caption{Configuración \textsl{bootstrap} simplificada.}
	\label{fig:img_bootstrap}
\end{figure}

El \textsl{bootstrap driver} consiste en un capacitor ($C_{BS}$), un diodo, y la circuitería interna del HIP4081A. Para garantizar el correcto funcionamiento del \textsl{bootstrap}, al encender el sistema, la secuencia de inicio del HIP4081A enciende las dos salidas de la parte inferior del puente H: ALO y BLO con el fin de encender $Q_3$ y $Q_4$ durante un tiempo que se conoce como periodo de refresco de \textsl{bootstrap}. De esta forma, los capacitores de \textsl{bootstrap} de ambos lados quedan conectados entre $12\:V$ y GND y se pueden cargar completamente. Durante este tiempo, las salidas a los \textsl{gates} AHO y BHO se mantienen en bajo continuamente lo que asegura que no se produzca corriente de cortocircuito durante el período nominal de refresco del \textsl{bootstrap}. Una vez finalizado, las salidas responden normalmente al estado de las señales de entrada de control.

Para comprender su funcionamiento se hará un breve análisis del sistema. Para ello, se parte de la suposición de que el sistema se encuentra en funcionamiento: en la figura \ref{fig:img_topologia-puenteH},  el transistor $Q_2$ y $Q_3$ se encuentran encendido (BHO = ALO = $V_{CC}$), y $Q_1$ y $Q_4$ apagados (AHO = BLO = $0\: V$). De esta manera, la corriente circula desde $Q_3$ hasta $Q_2$, pasando por la carga. Para simplificar el entendimiento del bootstrap, se utiliza la figura \ref{fig:img_bootstrap}, que muestra solo la mitad del puente y se enfoca en el capacitor de bootstrap que acompaña a $Q_1$. En ese caso, el capacitor $C_{BS}$ se carga a $12\: V$, ya que en un terminal tiene la fuente de $12\:V$ (a través del diodo $D_{BS}$) y el otro está conectado a GND por medio de $Q_3$.

Una vez que se apaga el transistor inferior, empieza a transcurrir el tiempo muerto. Debido a que la carga es inductiva, el valor instantáneo de la corriente mantiene su sentido y circula por los diodos antiparalelos del MOSFET inferior del lado A y el superior del lado B. Esto provoca que el \textsl{source} del transistor superior del lado A tenga una tensión negativa igual a la caída de tensión en directa del diodo antiparalelo de $Q_3$.  
...
\noindent Para el dimensionamiento del capacitor de \textsl{bootstrap} se tuvieron en cuenta sugerencias y procedimientos descriptos en \cite{HIP4081A_FN3659} y \cite{HIP4081A_AN9405}.
...
\begin{figure}[H]
	\centering
	\scalebox{0.8}{\input{tikz/diagrama_bloques_oscilador.tex}}
...
\begin{equation}\label{eq_potencia} 
	P = I^2 * R_T = 21.25^2 \:[A^2]* \frac{R_{ESR}}{n}
\end{equation}
...
La ecuación \ref{eq_salida_restador_hall} es válida únicamente para $I_L>=0\:A$. Para valores de $I_L<0\:A$, la salida será $V_{IL_{feed}}=0\:V$.
...
\begin{equation} \label{eq_salida_restador_hall_2}
	V_{IL_{feed}}=V_{sns}-2.5\:[V] =I_L*H_0 + 2.6\:[V] - 2.5\:[V] =I_L*H_0+0.1\:[V]
\end{equation}
...
\subsubsection{Circuito del oscilador auxiliar} \label{sec_oscilador_auxiliar}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{conmutacion-auxiliar.png}
	\caption{Circuito oscilador de frecuencia auxiliar.}
	\label{fig:img_frecuencia-auxiliar}
\end{figure}


%La señal de salida del circuito $V_{SW}$ es una onda pulsada con dos valores posibles de ciclo de trabajo ($d$) dependiendo del valor en su entrada ($V_h$):
%
%1- Cuando $V_h=0\:V$, entonces $d=90\%$. 
%
%2- Cuando $V_h=5\:V$, entonces $d=10\%$.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{Comparador-con-histeresis.png}
...
\begin{equation}
	V^+_{off}=2.5\:[V]*\frac{R_{7}}{R_{8}+R_{7}}
\end{equation}

2- Cuando $V_e<V^+$ entonces $V_h=5\:V$. Resulta:

\begin{equation}
	V^+_{on}=2.5\:[V]*\frac{R_{7}}{R_{8}+R_{7}} + 5\:[V]*\frac{R_{8}}{R_{8}+R_{7}}=V^+_{off}+5\:[V]*\frac{R_{8}}{R_{8}+R_{7}}
\end{equation}
...
\subsubsection{Circuito de conmutación completo}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.55]{etapa-conmutacion.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{conexiones-HIP-VSW.png}
...
\begin{table}[H]
	\begin{center}
		\begin{tabular}{| c | c | c |}
			\hline
			\backslashbox{Variables}{Resultados} & Resultado medido & Resultado teórico \\ \hline
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{simulacion_escalon_conmutacion.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{simulacion_corriente_negativa.png}
...
%Utilizando cursores en la simulación se midieron valores de interés y se armó la tabla asdasd.
%
%\begin{table}[H]
%	\begin{center}
%		\begin{tabular}{| c | c | c |}
...
%			$\overline{I_L}$ [mA] & $96.2$ & $96$ \\ \hline
%			$\Delta I_L$ [mA] & 505 & 500 \\ \hline
%			
%		\end{tabular}
%		\caption{Valores medidos en simulación y calculados teóricamente.}
%		\label{tab_mediciones_simulacion}
...
\noindent Para $Y_g = 4 \:mm $ se tiene una inductancia $L(4\:mm) = 16.44 \:mH$ , lo cual resulta en una frecuencia $F_{planta}= 1460\:Hz$ .
...
\noindent Al tomar las condiciones del problema en el punto de linealización con $Y_{0}=4\:mm$, resulta una inductancia $L = 7.55 \:mH + 8.89 \:mH$. Luego, al considerar la resistencia del bobinado $R_L=0.2\:\Omega$, se calcula la ubicación del polo:

\begin{equation} 
\omega _{polo} = \frac{0.2\:[\Omega]}{16.44 \:[mH]} = 12.17 \:[rad/s]
\end{equation}
...
\begin{table}[H]
	\begin{center}
		\begin{tabular}{| c | c | c | c | c |}
			\hline
			$Y_g\:[mm]$ & $L(Y_g)\:[mH]$ & $\Delta T\:[ms]$ & $F_{planta}\:[Hz]$ & $\omega _{polo}\:[rad/s]$\\ \hline
