...
A continuación se realizan simulaciones del circuito estimador conformado por los bloques derivador y rectificador+filtrado. Para ello se utilizó el software NL5 y se anexó el circuito estimador dentro del controlador de corriente. En la figura \ref{fig:img_simulacion_derivador+rectificador} se muestra el circuito utilizado para la simulación.
...
Se realizó una simulación del controlador de corriente + estimador utilizando $V_{IL_{ref}}=5\:V$, que resulta en una corriente media del electroimán $\overline{I_L}=30\:A$. En la figura \ref{fig:img_simulacion_derivador+rectificador_ss} se muestran las formas de onda de la corriente del electroimán, la salida del circuito derivador, y la salida del rectificador. 
...
En la forma de onda de corriente se puede observar que las pendientes de crecimiento y de decrecimiento no son simétricas. Esto es provocado por el efecto de la resistencia interna del electroimán, que se hace notorio cuando se trabaja con corrientes elevadas. Esta asimetría en las pendientes provoca que la salida del derivador no sea simétrica alrededor de $V_{op}$. Por lo tanto, como luego se rectifica esta señal pulsada alrededor de $V_{op}$, la señal resultante presentará variaciones. Para compensar este efecto se implementa la compensación de resistencia interna....

También se puede observar que la salida del derivador presenta un comportamiento ruidoso en su techo y su piso. Esto proviene de de la conmutación auxiliar, que también es derivada. Para solucionar este inconveniente se implementa un filtro pasa-bajos en la entrada del estimador., que se diseña a continuación... 

\colorbox{red}{tenemos que diseñar primero el LPF de la entrada y después la compensación de resistencia}
