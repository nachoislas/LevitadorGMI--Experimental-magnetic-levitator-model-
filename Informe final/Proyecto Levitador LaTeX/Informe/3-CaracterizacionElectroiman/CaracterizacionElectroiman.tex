...
\noindent Al tener una resistencia serie baja, la fuente de tensión activó la protección de corto circuito, de tal manera que la corriente en el electroimán quedó constante en $1\:A$. Utilizando la medición de tensión entregada por la fuente, cuyo resultado fue de $0,17 V$, se puedo calcular la resistencia del electroimán mediante la Ley de Ohm:
...
\noindent Se realizó una caracterización de la inductancia en función del gap de aire. Para realizarlo se utilizó un medidor LCR marca Instek y laminas de cartón de espesor conocido. La medición consta en ir apilando dichas laminas entre ambas piezas del electroimán, donde la suma total de los espesores de las laminas es $Y[mm]$ y luego tomar el valor de inductancia entregado por el medidor $(L(Y_{g})[mH])$. Siguiendo este procedimiento se obtuvieron los siguientes resultados:
...
\noindent Es posible observar en la expresión \ref{eq_transferencia_planta_m} que la masa del objeto modifica la ganancia del sistema, pero no la ubicación de los polos. Esto es importante tenerlo en cuenta al momento de escoger la estrategia de control.
