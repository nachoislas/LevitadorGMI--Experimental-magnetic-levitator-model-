...
Por lo tanto, se decide utilizar los MOSFET IPB160N04 ya que, ademas de estar presentes en el prototipo anterior, cumplen con los requerimientos de corriente: soportan una $I_{D_{MAX}}=160\:A$, una tensión $V_{DS_{MAX}} = 40\:V$ y una $R_{ds} = 1.6 \:m \Omega$. Estos transistores son de tipo N y, para que estén en estado de conducción, debe aplicarse una tensión entre gate y source mínima de $V_{gs}= 3\:V$.
...
\colorbox{red}{Este parrafo creo qeu se repite...}
...
El controlador por histéresis no asegura que haya una conmutación en un tiempo constante , esto puede ocurrir cuando el sistema exige un cambio en la corriente media. Cuando esto ocurre por un tiempo considerable el capacitor de boostrap se descarga a travez de la resistencia del gate (Rgs), si este tiempo es considerable al querer conmutar nuevamente, el gate no tendr'ia la carga suficiente para activar al $MOSFET$ superior por lo tanto la conmutación no se podrá efectuar. Por este motivo se decidió superponer una conmutación auxiliar constante que no afecte el funcionamiento del sistema con el objetivo de evitar la descarga de los capacitores de boostrap.
