...
\begin{tikzpicture}[auto, node distance=1cm,>=latex']
	...
	\node [block, right=of suma_externa, text width=2.5cm] (externo) {COMP. EXTERNO};
	\node [sum, right=of externo] (suma_interna) {+};
	\node[block, right=of suma_interna, text width=2.5cm] (interno) {COMP. INTERNO};
...
%	% Once the nodes are placed, connecting them is easy. 
	...
	\draw [draw,->] (externo) -- node{$Vref2??$} (suma_interna);
